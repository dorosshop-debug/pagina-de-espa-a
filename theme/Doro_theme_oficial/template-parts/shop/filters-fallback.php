...
$exclude_ids = array();
if ( function_exists( 'doroshopping_is_uncategorized_term' ) ) {
...
if ( taxonomy_exists( 'product_cat' ) ) {
	...
	// Filtrar "Sin categorizar" por nombre/slug.
...
/**
 * ¿La categoría (o un hijo) está activa en la vista actual?
 *
 * @param WP_Term $term Term.
 * @param int     $current_id Current term id.
 * @param int[]   $ancestors Ancestor ids.
 * @return bool
 */
									...
									<?php if ( $child_has ) : ?>
										...
										>
											<?php foreach ( $grandchildren as $gchild ) : ?>
												<?php $g_current = $current_term_id === (int) $gchild->term_id; ?>
												<li class="doro-shop__cat-item doro-shop__cat-item--grand">
													<a
														href="<?php echo esc_url( get_term_link( $gchild ) ); ?>"
		...
		<div class="doro-shop__widget">
			...
			<ul class="<?php echo esc_attr( $list_class ); ?>">
				<?php foreach ( $terms as $term ) : ?>
					...
					$swatch    = '';
					if ( $is_color ) {
						// Intentar color desde meta de término (plugins comunes) o slug.

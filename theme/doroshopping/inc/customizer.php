...
function doroshopping_customizer_css() {
    $orange = get_theme_mod( 'doroshopping_color_orange', '#f8942d' );
    $dark   = get_theme_mod( 'doroshopping_color_dark', '#1a1a1a' );
    $grey   = get_theme_mod( 'doroshopping_color_grey_bg', '#f5f5f5' );
    $red    = get_theme_mod( 'doroshopping_color_red_accent', '#e53935' );
    $footer = get_theme_mod( 'doroshopping_color_footer_bg', '#000000' );

    $css = ':root{';
    $css .= '--color-orange:' . sanitize_hex_color( $orange ) . ';';
    $css .= '--color-dark:' . sanitize_hex_color( $dark ) . ';';
    $css .= '--color-grey-bg:' . sanitize_hex_color( $grey ) . ';';
    $css .= '--color-red-accent:' . sanitize_hex_color( $red ) . ';';
    $css .= '}';
    $css .= '.site-footer{background:' . sanitize_hex_color( $footer ) . ';}';
    $css .= '.site-fab-cart,.site-fab-cart__count,.cart-modal__checkout,.doro-buybox__buy-now{--doro-cta:' . sanitize_hex_color( $red ) . ';}';
    $css .= '.site-fab-cart{color:var(--doro-cta);}';
    $css .= '.site-fab-cart__count,.cart-modal__checkout,.doro-buybox__buy-now{background:var(--doro-cta);}';
